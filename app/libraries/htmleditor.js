(function(addon) {
    ...
    UI.component('htmleditor', {
        ...
        init: function() {
            ...
            // this.CodeMirror = this.options.CodeMirror || window.CodeMirror;
            this.buttons    = {};
            ...
            this.editor = window.CodeMirror.fromTextArea(this.element[0], this.options.codemirror);
            ...
            // autocomplete
            if (this.options.autocomplete && window.CodeMirror.showHint && window.CodeMirror.hint && window.CodeMirror.hint.html) {

                this.editor.on('inputRead', UI.Utils.debounce(function() {
                    var doc = $this.editor.getDoc(), POS = doc.getCursor(), mode = window.CodeMirror.innerMode($this.editor.getMode(), $this.editor.getTokenAt(POS).state).mode.name;
                    ...
                    if (mode == 'xml') { //html depends on xml
                        ...
                        if (token.string.charAt(0) == '<' || token.type == 'attribute') {
                            window.CodeMirror.showHint($this.editor, window.CodeMirror.hint.html, { completeSingle: false });
                        }
                    }
                }, 100));
            }
